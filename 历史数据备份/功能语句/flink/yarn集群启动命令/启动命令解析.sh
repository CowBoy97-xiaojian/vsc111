...
#NO1
-Dyarn.ship-files 是 Apache Flink 在启动时的一个启动参数，用于将本地文件分发到 YARN 集群的计算节点上。

在基于 YARN 的 Flink 集群中，通常需要将 Flink 作业使用到的配置文件、依赖文件等分发到 YARN 的计算节点上，以便作业在计算节点上能够访问到这些文件。

-Dyarn.ship-files 参数用于指定需要分发的文件列表。它会告诉 Flink 将指定的文件分发到 YARN 集群中的计算节点上，以便作业执行时可以使用这些文件。参数的使用方式如下：

-Dyarn.ship-files=file1,file2,...
其中，file1,file2,... 表示需要分发的文件列表，多个文件之间用逗号分隔。可以使用绝对路径或相对路径指定文件的位置。

例如，假设有一个名为 flink-conf.yaml 的配置文件需要分发到 YARN 计算节点上，可以使用以下启动参数：

-Dyarn.ship-files=/path/to/flink-conf.yaml
这样，Flink 在启动时会将 flink-conf.yaml 文件分发到 YARN 计算节点上的相应位置，使得作业可以访问到该配置文件

#NO2
-Dyarn.flink-dist-jar 是 Apache Flink 在启动时的一个启动参数，用于指定 Flink 分发包（distribution）的位置和名称。Flink 分发包是指包含了 Flink 运行时环境以及依赖库等的 JAR 文件。

在基于 YARN 的 Flink 集群中，通常需要将 Flink 分发包分发到 YARN 集群中的计算节点上，以便作业在计算节点上执行时可以使用 Flink 运行时运行环境。-Dyarn.flink-dist-jar 可以帮助我们告诉 Flink 分发包的位置和名称，并让 Flink 启动时自动将分发包分发到 YARN 集群中的计算节点上。

参数的使用方式如下：

-Dyarn.flink-dist-jar=file:///path/to/flink-dist.jar
其中，file:///path/to/flink-dist.jar 指定了 Flink 分发包的位置和名称。可以使用绝对路径或相对路径指定文件的位置。

例如，假设有一个名为 flink-dist.jar 的 Flink 分发包需要分发到 YARN 计算节点上，可以使用以下启动参数：

-Dyarn.flink-dist-jar=file:///path/to/flink-dist.jar
这样，Flink 在启动时会将 flink-dist.jar 文件自动分发到 YARN 计算节点上的相应位置，以便作业可以使用 Flink 运行时环境。

#NO3
flink-dist_2.11-1.14.2.jar 是 Apache Flink 的分发包（distribution package）。它是一个包含了 Flink 运行时环境以及相关依赖库的 JAR 文件。

具体来说，flink-dist_2.11-1.14.2.jar 是针对 Scala 2.11 平台的 Apache Flink 1.14.2 版本的分发包。它包含了 Flink 的核心功能和库，包括流处理和批处理的 API、状态管理、容错机制、连接器、序列化库、作业管理和调度等组件。

利用这个分发包，你可以在 Flink 程序中引用相关的类和库，构建和运行 Flink 应用程序。

需要注意的是，flink-dist_2.11-1.14.2.jar 是 Flink 的分发包，而不是用于开发 Flink 应用程序的 API JAR 包。如果你希望开发 Flink 应用程序，你需要引入针对具体语言和版本的 Flink API JAR 和相应的依赖库。
...
#启动命令全参数表
[app@yarnflink-1 lib]$ /data1/flink-1.16.3/bin/flink
Picked up JAVA_TOOL_OPTIONS: -Duser.timezone=UTC -Dfile.encoding=UTF-8 -Dsun.jnu.encoding=UTF-8 -Dlog4j2.formatMsgNoLookups=true
