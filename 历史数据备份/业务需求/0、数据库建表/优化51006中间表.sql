CREATE TABLE webtrends.event_hi_dcslog_update_local on cluster cluster_gio_with_shard
(
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/webtrends.event_hi_dcslog_update_local', '{replica}')
PARTITION BY (dt, hour)
ORDER BY (event_id, gio_id)
SETTINGS index_granularity = 8192, storage_policy = 'multiple_disk';


CREATE TABLE webtrends.event_hi_dcslog_update_all on cluster cluster_gio_with_shard
as webtrends.event_hi_dcslog_update_local
ENGINE = Distributed('cluster_gio_with_shard', 'webtrends', 'event_hi_dcslog_update_local', abs(javaHash(toString(gio_id))));


CREATE INDEX index_dcsid ON webtrends.event_hi_dcslog_update_local(dcsid) TYPE bloom_filter GRANULARITY 65536;