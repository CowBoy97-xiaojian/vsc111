CREATE TABLE webtrends.event_hi_dcslog_imp_local on cluster cluster_gio_with_shard
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/webtrends.event_hi_dcslog_imp_local', '{replica}')
PARTITION BY (dt, hour)
ORDER BY (dcsdat,wt_mobile)
SETTINGS index_granularity = 8192, storage_policy = 'multiple_disk';


CREATE TABLE webtrends.event_hi_dcslog_imp_all on cluster cluster_gio_with_shard
as webtrends.event_hi_dcslog_imp_local
ENGINE = Distributed('cluster_gio_with_shard', 'webtrends', 'event_hi_dcslog_imp_local', rand());
