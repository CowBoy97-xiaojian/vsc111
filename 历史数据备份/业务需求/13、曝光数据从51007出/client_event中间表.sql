CREATE TABLE webtrends.event_hi_client_test_local on cluster cluster_gio_with_shard
(   event_key String,
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/{shard}/webtrends.event_hi_client_test_local', '{replica}')
PARTITION BY (dt, hour, dcsid)
ORDER BY (dcsdat, user_agent,event_key, event_id, gio_id)
SETTINGS index_granularity = 8192, storage_policy = 'multiple_disk';

truncate table webtrends.event_hi_client_test_local on cluster cluster_gio_with_shard;

drop table  IF EXISTS webtrends.event_hi_client_all on cluster cluster_gio_with_shard;

DROP TABLE IF EXISTS webtrends.event_hi_client_test_local on cluster cluster_gio_with_shard;


CREATE TABLE webtrends.event_hi_client_test_all on cluster cluster_gio_with_shard
as webtrends.event_hi_client_test_local
ENGINE = Distributed('cluster_gio_with_shard', 'webtrends', 'event_hi_client_test_local', rand());