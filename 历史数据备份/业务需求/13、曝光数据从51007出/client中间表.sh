CREATE TABLE webtrends.event_hi_client_local on cluster cluster_gio_with_shard
...
PARTITION BY (dt, hour, dcsid)
ORDER BY (dcsdat, user_agent, event_id, gio_id)
SETTINGS index_granularity = 8192, storage_policy = 'multiple_disk';

truncate table webtrends.event_hi_client_local on cluster cluster_gio_with_shard;

drop table  IF EXISTS webtrends.event_hi_client_local on cluster cluster_gio_with_shard;

DROP TABLE IF EXISTS webtrends.event_hi_client_local on cluster cluster_gio_with_shard;

drop table  IF EXISTS webtrends.event_hi_client_all on cluster cluster_gio_with_shard;

alter table webtrends.event_hi_client_local  add column event_key String;


CREATE TABLE webtrends.event_hi_client_all on cluster cluster_gio_with_shard
as webtrends.event_hi_client_local
ENGINE = Distributed('cluster_gio_with_shard', 'webtrends', 'event_hi_client_local', rand());