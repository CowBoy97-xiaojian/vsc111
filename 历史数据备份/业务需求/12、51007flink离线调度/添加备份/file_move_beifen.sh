...
target_path=/data2/udbac/output_ck/"$file_type"_daily
date_time_path=$2
date_path=$3
date_hour=$4
...
if [ $file_ct -gt 0 ]; then
	...
	while [ "$md5_src" != "$md5_trg" ]; do
      ...
      if [ "$date_hour" = "03" ]; then
            ssh $ip "rm -rf $source_path/$delete_date/"
      fi
      if [ "ssh 10.253.182.84 -d ${copy_path}/${date_path}" ]; then
            ssh 10.253.182.84 "mkdir -p ${copy_path}/${date_path}"
      fi
      scp ${target_path}/${date_path}/$target_file 10.253.182.84:$copy_path/$date_path
...
if [ "$date_hour" = "00" ]; then
      ssh 10.253.182.84 "rm -rf $copy_path/$dalete_date/"
fi
time2=$(date "+%Y-%m-%d %H:%M:%S")
echo "完成所有文件迁移_当前时间"$time2