...
sh $file $daytime $hour | sed 's/"//g' | sed "s/'/\"/g" | split -a 3 --additional-suffix=.dat -d -l 800000 -d --numeric-suffixes=1 - 510_"$hour"_
...
filecount=1
for g in ${origz};do
  cat $g | iconv -c -t GBK | sed "s/\x01/\x80/g"  >  a_10000_"$hour"_"$5"_00_"$filecount".dat
...
for i in $(ls *.dat.gz); do
  if [ $filecount1 -lt 10 ];then
     mv $i a_10000_"$day"_"$5"_00_00"$filecount1".dat.gz
  elif [ $filecount1 -lt 100 ];then
     mv $i a_10000_"$day"_"$5"_00_0"$filecount1".dat.gz
  else
     mv $i a_10000_"$day"_"$5"_00_"$filecount1".dat.gz
  fi
