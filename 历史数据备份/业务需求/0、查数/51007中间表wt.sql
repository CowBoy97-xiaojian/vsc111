...
alter table webtrends.event_hi_client_local on cluster cluster_gio_with_shard drop partition('${DT}','${Hour}','dcs0cxkozfonmgrs8gfnw57g1_2e4p');
insert into table webtrends.event_hi_client_all
select 
...
decodeURLComponent(attributes['WT_ti']) as wt_ti,
...
decodeURLComponent(attributes['cs_referer']) as cs_referer, 
decodeURLComponent(attributes['WT_es']) as wt_es,
...
CASE WHEN wt_event = 'sdk_loaded' AND data_source_id = 'dcsgswzxehonmgrc8hz5w67g1_9o7q' THEN concat('APP_ios_v',attributes['WT_av']) 
...
attributes['channelId'] as channelid,
...
decodeURLComponent(attributes['WT_clientID']) as clientid,
attributes['pageId'] as  pageid,
attributes['sellerId'] as sellerid,
decodeURLComponent(concat(attributes['ck_id'],':',attributes['WT_vtvs'])) AS ss_id,
attributes['extendId'] as extendid,
...
WHERE dt = '${DT}'
and hour = '${Hour}'
and data_source_id = 'dcs0cxkozfonmgrs8gfnw57g1_2e4p';
"
