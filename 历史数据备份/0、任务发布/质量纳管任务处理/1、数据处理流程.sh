...
insert overwrite table ham_jituan.dim_client_work_order_d_tmp
select 
if(length(marketing_id) = 14, split(area_location,'-')[1] ,marketing_id) as marketing_id,
area_name as area_location,
area_name 
from ham_jituan.dim_client_work_order_d
where dt = '${DT}' 
and if(length(marketing_id) = 14, split(area_location,'-')[1] ,marketing_id) is not null
and area_name in ('流量专区','权益专区','套餐专区','终端专区');

insert into table ham_jituan.dim_client_work_order_d_tmp
select 
marketing_id,
split(area_location,'-')[0]  as area_location,
area_name 
from ham_jituan.dim_client_work_order_d
where dt = '${DT}'
and marketing_id is not null
and area_name = '业务楼层区域'
and split(area_location,'-')[0] in ('流量专区','权益专区','套餐专区','终端专区');
...
数据格式处理
1、event=H5PageShow      && 5个页面内的es匹配
    匹配到es--->falg=1
2、event！=H5PageShow    && 5个页面内的markid匹配
    匹配维表markid--->flag=0

根据es做分组数据计算
    按照cid和dcsdat排序
    取相邻两条cid相等的数据,flag=0为开始,flag=1为结束。dcsdat相减(1-0)取平均值。