...
session AS ss_id,
-- split(ip,'\\|')[0] as ip,
-- split(ip,'\\|')[1] as ip_prov,
-- --case when split(ip,'\\|')[1]='中国' then split(ip,'\\|')[2] else split(ip,'\\|')[1] end  as ip_prov,
-- split(ip,'\\|')[2] as ip_city,
if(instr(ip,'|')>0,split(ip,'\\|')[0],ip) as ip,
if(instr(ip,'|')>0,split(ip,'\\|')[1],parse_ip(if(instr(ip,'|')>0,split(ip,'\\|')[0],ip)).prov) as ip_prov,
if(instr(ip,'|')>0,split(ip,'\\|')[2],parse_ip(if(instr(ip,'|')>0,split(ip,'\\|')[0],ip)).city) as ip_city,
...
--2023年10月24日--channel_id改为优先取WT_channelid，次取wt_es里的channelid,再取$query里的 start--
...
decode_url(nvl(nullif(split(attributes['WT_channelid'],'%')[0], ''),nullif(substr(regexp_extract(coalesce(parse_url(attributes['WT_es'], 'QUERY'),split(attributes['WT_es'],'\\?')[1],attributes['$query']), 'channelId=([0-9a-zA-z]+)', 1),0,12),''))) AS channel_id,
--2023年10月24日--channel_id改为优先取WT_channelid，次取wt_es里的channelid,再取$query里的 end--
--2023年10月24日--page_id改为优先取wt_pageid，次取wt_es里的pageid,再取$query里的 start--
...
decode_url(substr(nvl(nullif(attributes['WT_pageid'], ''), nullif(regexp_extract(coalesce(parse_url(attributes['WT_es'], 'QUERY'),split(attributes['WT_es'],'\\?')[1],attributes['$query']), 'pageId=([0-9a-zA-z]+)', 1),'')), 0, 19)) AS page_id,
--2023年10月24日--page_id改为优先取wt_pageid，次取wt_es里的pageid,再取$query里的 end--
...
--2023年10月24日--seller_id改为优先取WT_sellerid，次取wt_es里的seller_id,再取$query里的 start--
...
decode_url(nvl(nullif(attributes['WT_sellerid'], ''), nullif(regexp_extract(coalesce(parse_url(attributes['WT_es'], 'QUERY'),split(attributes['WT_es'],'\\?')[1],attributes['$query']), 'sellerId=([0-9a-zA-z]+)', 1),''))) AS seller_id,
--2023年10月24日--seller_id改为优先取WT_sellerid，次取wt_es里的seller_id,再取$query里的 end--
...
where dt = '${DT}'
and hour = '${HH}'
and data_source_id in ('8a568ccb742bdc43','aa410d5cd21666f5','a1307114a76cd375','a6381779603b656a','913e6dc4915d470c','8643086d472dea68','82213179b9aea392','aba9de4ce446b2d2','ac81a55cdf7074d5','999264cd2e773538','beb5471c243b2971','b5b9ccb1f69a35b9','a1f48d9ff4f42571','b508a809cbbddd0b','8e76b40944f88ee5','9f8770a1dbd09047')
