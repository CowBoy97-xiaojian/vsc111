...
#clickhouse-client -h 10.104.82.67 -n --receive_timeout=3600 --query="
insert into table webtrends.event_hi_client_update_all
select 
...
--aes_function(if(match(user, '^1[3-9]\\d{9}$'), user, '')) as wt_mobile,
if(match(user, '.*==$'), aes_function(user), user) as wt_mobile,
