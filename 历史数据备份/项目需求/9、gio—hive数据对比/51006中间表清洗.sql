...
select 
coalesce(case when regexp(user, '^[0-9]{11}$') then user when regexp(user, '.*==$') then user end, first_value(case when regexp(user, '^[0-9]{11}$') then user when regexp(user, '.*==$') then user end)
...
if(instr(domain,'.')>0,if(regexp(domain,'^[0-9]+[.]+[0-9]+[.]+[0-9]+[.]+[0-9]+$'),concat('http://',coalesce(domain,''),coalesce(attributes['$path'],'')),concat('https://',coalesce(domain,''),coalesce(attributes['$path'],''))),domain) as url,
...
if(decode_url(split(cast(coalesce(attributes['WT_channelid'],'') as string),'%')[1]) = '--',nullif(substring(regexp_extract(decode_url(attributes['$query']), 'channelId=([0-9a-zA-z]+)'),1,12),''),decode_url(split(cast(coalesce(attributes['WT_channelid'],'') as string),'%')[1])) as channel_id,
if(decode_url(attributes['WT_pageid']) = '--',substring(nullif(regexp_extract(decode_url(attributes['$query']), 'pageId=([0-9a-zA-z]+)'),''), 1, 19),decode_url(attributes['WT_pageid'])) as page_id,
decode_url(attributes['WT_input_xm']) AS input_xm,
decode_url(attributes['WT.input_sfz'])  AS input_sfz,
decode_url(attributes['WT.input_dz']) AS input_dz,
decode_url(attributes['WT_mc_ev']) AS mc_ev,
if(decode_url(attributes['WT_sellerid']) ='--',regexp_extract(decode_url(attributes['$query']), 'sellerId=([0-9-a-zA-z]+)'),decode_url(attributes['WT_sellerid'])) as seller_id,
...
unix_timestamp(client_time) * 1000 + CAST(split(CAST(client_time as string), '\\.')[1] as Int) AS cli_time,
...
concat(if(instr(domain,'.')>0,if(regexp(domain,'^[0-9]+[.]+[0-9]+[.]+[0-9]+[.]+[0-9]+$'),concat('http://',coalesce(domain,''),coalesce(attributes['$path'],'')),concat('https://',coalesce(domain,''),coalesce(attributes['$path'],''))),domain),coalesce(concat('?',attributes['$attributes']),'')) as complete_url,
...
'--' AS tbd,
coalesce(attributes['WT_dcsdat'],'') AS dcsdat,
...
concat(if(instr(domain,'.')>0,if(regexp(domain,'^[0-9]+[.]+[0-9]+[.]+[0-9]+[.]+[0-9]+$'),concat('http://',coalesce(domain,''),coalesce(attributes['$path'],'')),concat('https://',coalesce(domain,''),coalesce(attributes['$path'],''))),domain),coalesce(concat('?',attributes['$query']),'')) as es,
...
from (
  select 
    ...
    merged_gio_id,
    session,
    str_to_map(regexp_replace(attributes, '^\\{"(.*)"\\}$', '$1'),'\",\"','\":\"') as attributes,
    attributes as attributes_str,
    package,
    platform,
    referrer_domain,
    utm_source,
  ...
  where dt = '${DT}'
  and hour = '${HH}'
  and data_source_id in ('aa410d5cd21666f5','a1307114a76cd375','a6381779603b656a','913e6dc4915d470c','8643086d472dea68','82213179b9aea392','aba9de4ce446b2d2')
) tb1
;