...
if [ "$#" -ne 4 ]; then
  echo "参数列表错误：（1）日期（yyyymmdd）（2）应当是CM002 （3）省份编码（4）省份拼音" >&2
  exit 1
fi
...
cd $1/$2/$4
echo "拉取数据文件"
hdfs dfs -get /user/hive/warehouse/ham_jituan.db/ads_hachi_"${2,,}"/prov=$4/*
...
echo "文件转换分割"
cat new_*.txt |awk '{print NR "#_#" $0}'| sed "s/\\x23\\x5F\\x23/\\x80/g" | split -a 3 --additional-suffix=.dat -d -l $CNT --numeric-suffixes=1 - a_10000_"$1"_"$2"_"$3"_00_
rm -f new_*.txt
