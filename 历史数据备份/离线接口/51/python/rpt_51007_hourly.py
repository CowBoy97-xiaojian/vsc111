...
with DAG('rpt_51007_hourly',
    ...
    export_51007_hourly = BashOperator(
        ...
        sh /home/udbac/bin/hachi_bigtable_07_hourly.sh {dfmt} JZYY_XTB2_51007 {DT} {HH}
...
#        fi
    ...
    sensor_etl_client_gio_hourly = ExternalTaskSensor(
        ...
        external_dag_id='rpt_51007_gio_hourly',
        external_task_id='export_51007_gio_hourly',
        timeout=10800,
        allowed_states=['success'],
        mode="reschedule")
    ...
    transform_name_daily = BashOperator(
        ...
        then
           ...
           flag_sum=$(($(ls -l -R /data/output/51007_hourly/{dfmt} | grep flag | wc -l)+$(ls -l -R /data/output/51007_gio_hourly/{dfmt} | grep flag | wc -l)))
           if [ '48' -eq $flag_sum ]
           then
             echo "开始汇总数据"
             sh /home/udbac/bin/transform_name_cp_51007.sh {dfmt} {DT}
           else
             #等待30分钟，再进行重跑
             echo "小时数据未成功全部导出,以下是完成小时列表:"
    ...
    push_51007 = BashOperator(
        ...
        cd /data/output/51007_hourly/{dfmt}/
        if [ '23' -eq {HH} ];then
          sh /home/udbac/bin/new_push_51007.sh {DT}
           ct=$(ls *.dat.gz | wc -l)
    ...
    sensor_etl_client_hourly >> rpt_hachi_JZYY_XTB2_51007_hourly >> export_51007_hourly >> sensor_etl_client_gio_hourly  >> transform_name_daily  >> push_51007
