...
Njob=5 #任务总数  
Nproc=5 #数据写出最大并发进程数
file=/home/udbac/bin/ckout_51006_sql.sh #sql脚本文件
...
hdfs dfs -put /home/udbac/output/data_h5/data_ck_"$ck_dt"_"$hour"_"$ip".csv /user/hive/ck_out/h5/
rm -f /home/udbac/output/data_h5/data_ck_"$ck_dt"_"$hour"_"$ip".csv
...
sql_del="alter table ham.ods_dcslog_event_gio_all drop partition(dt='$hive_dt',hour='$hour')"
echo "先删除$hive_dt，$hour分区数据"
beeline -n udbac -u jdbc:hive2://master01:10000/ham -e "$sql_del"

sql="load data inpath 'hdfs://udbachdp1/user/hive/ck_out/h5/data_ck_${ck_dt}_${hour}_*' into table ham.ods_dcslog_event_gio_all partition(dt='$hive_dt',hour='$hour')"
echo "开始hivesql导入数据"
