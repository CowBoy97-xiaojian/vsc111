...
#统计小时导出文件行数
export_sum=$[$export_sum + $(cat /home/udbac/output/51006_hourly/${dt}/${dt}_${i}/export_${dt}_JZYY_XTB1_51006_${i}.log)]
...
#统计小时导出文件行数
export_sum=$[$export_sum + $(cat /home/udbac/output/51006_gio_hourly/${dt}/${dt}_${i}/export_${dt}_JZYY_XTB1_51006_gio_${i}.log)]
...
from ham.ads_rpt_hachi_51006_gio_hourly
where dt='${dfmt}'
union all
select count(1) as cnt
from ham.ads_rpt_hachi_51006_hourly
where dt='${dfmt}'
) tb1
;
...
INSERT OVERWRITE TABLE ham_jituan.data_job_monitoring partition (dt = '${dfmt}',hour = '23',job_name = 'transform_51006_daily')
values ('51006','${start_time}','${end_time}','${filecount1}','${export_sum}','${hive_sum}','${data_consistency}','运行完毕');
